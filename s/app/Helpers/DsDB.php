...
class DsDB
{
		...
		if (!self::isItemInCart($params)) {
			...
			foreach ($items as $item) {
				DB::insert("INSERT INTO cart (sessionID,pid,price,qty,weight, length, width, height) VALUES (?,?,?,?,?,?,?,?)",
					[$params[0], $params[1], round($item->price, 2), $params[2], $item->weight, $item->length, $item->width, $item->height]);
			}
		}
	}
	...
	public static function updateCheckoutTemp($sid, $data)
	{
        ...
        if (isset($data['ShippingMethod']) && isset($data['EstShipCost'])) {
            $estShipCost = $data['EstShipCost'][$data['ShippingMethod']];
            unset($data['EstShipCost']);
            $data['EstShipCost'] = $estShipCost;
        }
    ...
    public static function addAuthCredentials($login, $key)
    {
        $crypt = new \Illuminate\Support\Facades\Crypt;

        DB::insert("INSERT INTO settings (setting_key, setting_value) VALUES(?,?)", ['authnet_login', $crypt::encryptString($login)]);
        DB::insert("INSERT INTO settings (setting_key, setting_value) VALUES(?,?)", ['authnet_key', $crypt::encryptString($key)]);
    }
}