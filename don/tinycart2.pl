...
</TD><TD COLSPAN="2" ALIGN="RIGHT">},sprintf("%0.2f",$__TAX_STATES__{uc($shipstate)}*$subtotal),qq{</TD></TR>
  <TR><TD COLSPAN="4"><HR></TD></TR>
  <TR><TD COLSPAN="2" ALIGN="RIGHT">Total</TD><TD
COLSPAN="2" ALIGN="RIGHT"><B><FONT COLOR="Blue">},
sprintf("%0.2f",$__TAX_STATES__{uc($shipstate)}*$subtotal+$subtotal+$shipcost),qq{</B></FONT></TD></TR>};
  }
...
BGCOLOR="#ffcc66">
  ...
  while (@row=$sth->fetchrow_array) {
	...
	print qq{<TR><TD><INPUT NAME="qty" VALUE="$qty" SIZE=4 MAXLENGTH=4></TD>},
		...
		qq{<TD>\$},sprintf("%0.2f",$price),qq{</TD>},
...
sub view_cart {
 ...
 print $cgih->end_form;
 print qq{<INPUT style="width:120" TYPE="BUTTON" VALUE="Continue Shopping" onclick="javascript:window.location='https://www.ds-sewing.com';">};
 print $cgih->end_html;
}
}
...
sub shipping_form  {
 ...
 my @required_bill = ('ContactPhone','BillFirstName','BillAddress1','BillCity','BillState','BillZip','EmailAddress');
...
#--
 ...
 <TR><TD><B>Zip/Postal Code</B></TD><TD><INPUT SIZE=5 MAXLENGTH=5 NAME="ShipZip" VALUE="$shipzip"></TD></TR>
...
#-- actually work next pages magic
 ...
 my ($shipments,$sth,$total_weight,$temp_weight,$max_weight,$oversized, $result,@row,$estes,$ups,$amer,$qty);

 $sth = $dbh->prepare("SELECT cart.weight,cart.qty,catalog.oversized FROM cart,catalog WHERE sessionID = $sid and catalog.pid = cart.pid");
 $result = $sth->execute;
...
if ($temp_weight>0) 
{
 ...
 if ($max_weight > 150 || $oversized!=0 ) { $ups = undef; $shipmethod='ESTES';} else { $shipmethod='UPSGROUND'; }

 $estes = estes_rate("06512",$shipzip,$total_weight,55) + ($shipments * $__SHIP_INCREASE__);
 ...
 print qq{<TR><TD>Common Freight Carrier</TD><TD>};
 if ($estes>$__SHIP_INCREASE__) {
  print '$',sprintf("%0.2f",$estes),qq{</TD><TD> <INPUT TYPE="RADIO" NAME="ShipMethod" VALUE="ESTES"};
  if ($shipmethod eq 'ESTES') { print "CHECKED"; };
  print qq{></TD></TR>};
 } else {
...
#print qq{<TR><TD>American Freight</TD><TD>};
#if ($amer>0) {
#  print '$',$amer,qq{</TD><TD><INPUT TYPE="RADIO" NAME="ShipMethod" VALUE="AMER"};
#  if ($shipmethod eq 'AMER') { print "CHECKED"; }
#  print qq{></TD></TR>};
# } else {
#  print qq{Not Avail</TD><TD>&nbsp;</TD>};
# }
...
VALUE="$billad1"></TD></TR>
 ...
 <TR><TD><B>Zip/Postal Code</B></TD><TD><INPUT SIZE=5 MAXLENGTH=5 NAME="BillZip" VALUE="$billzip"></TD></TR>
 <TR><TD><B>Contact Phone #</B></TD><TD><INPUT SIZE=32 MAXLENGTH=32 NAME="ContactPhone" VALUE="$contactphone"></TD></TR>
 <TR><TD><B>Email Address</B></TD><TD><INPUT SIZE=32 MAXLENGTH=255 NAME="EmailAddress" VALUE="$emailaddress"></TD></TR>
 </TABLE>
};
...
<TABLE BORDER="0" CELLSPACING="2" CELLPADDING="1" BGCOLOR=#ffcc66>
 ...
 <TR>
  <TD>Last 3 Digits On Back of Cart (CVV2)</TD><TD><INPUT SIZE=3 MAXLENGTH=3 NAME="CVV2" VALUE="$cvv2">
  <font size=-1>
...
#- check validity of credit card
 ...
 my ($shipments,$sth,$total_weight,$temp_weight,$max_weight,$result,@row,$estes,$ups,$qty);
...
#-- local variables if you will :)
 ...
 $sth->bind_param(19,$cvv2);
 $sth->bind_param(20,$shipmethod);
 $sth->bind_param(21,$shipcost);
 $sth->bind_param(22,$contactphone);
 $sth->bind_param(23,$emailaddress);
 $sth->bind_param(24,$tax);
 ...
 $sth = $dbh->prepare("insert order_detail select $orderid,pid,price,weight,qty from cart where sessionid=$sid");
 $result = $sth->execute; 
 if (!$result) { die($sth->errstr); }    
 $sth->finish;
 ...
 print qq{
	...
	<img src="https://www.commission-junction.com/track/receive.dll?AMOUNT=$subtotal&CID=663613&OID=$orderid&TYPE=SALE&METHOD=IMG&KEEP=NO" height="1" width="1">
...
sub main {
 $cgih = new CGI;
 $dbh = DBI->connect('DBI:ODBC:DSSEWING','sa','') or error('unable to connect');
 $dbh->{'AutoCommit'} = 1;
