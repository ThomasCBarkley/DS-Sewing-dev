...
print qq{<TR BGCOLOR="#c0c0c0"><TD><B>Qty</B></TD>},
	...
	qq{<TD ALIGN="CENTER"><B>Price</B></TD>},
  ...
  while (@row=$sth->fetchrow_array) {
	...
	if ($counter % 2 == 0) { print qq{<TR BGCOLOR="#ffffff">}; } else { print qq{<TR BGCOLOR="#F0F0F0">}; }
		...
		if ($price!=$discount) {		
			print sprintf("%0.2f", $discount * $qty),
 ...
 # $subtotal=$sth->fetchrow_array;
  ...
  print qq{<TR>
					<TD COLSPAN="2" ALIGN="LEFT">
						...
						<a href="http://www.ds-sewing.com/legal.htm" target="_blank">http://www.ds-sewing.com/legal.htm</a>
					</TD>
...
ALIGN="RIGHT" BGCOLOR="#F0F0F0">\$},sprintf("%0.2f",$subtotal),qq{</TD></TR>
  <TR>
					<TD COLSPAN="2" ALIGN="LEFT">
						...
						<a href="http://www.ds-sewing.com/forms/payment.html" target="_blank">http://www.ds-sewing.com/forms/payment.html</a>
					</TD>  
...
sub shipping_form  {
 ...
 my @required_ship = ('ShipContactName','ShipAddress1','ShipCity','ShipState','ShipZip');
...
print qq{
 ...
 if ($estes>($shipments * $__SHIP_INCREASE__)) {
  print '$',sprintf("%0.2f",$estes),qq{</TD><TD> <INPUT TYPE="RADIO" NAME="ShipMethod" VALUE="LTL"};
...
# $sth->bind_param(36,$tax);
 ...
 '$paymentmethod','$creditcardnumber','$exp_date','$cvv2','$shipmethod',CONVERT(money,'$shipcost'),'$contactphone','$emailaddress',CONVERT(money,'$tax'),'0')
 ";
...
#  $cart_content =  $cart_content.'<TR BGCOLOR="#c0c0c0"><TD COLSPAN="5" ALIGN="CENTER"><B>Shopping Cart</B></TD></TR>';
  ...
  $cart_content = $cart_content.'<TD ALIGN="CENTER"><B>Price</B></TD>';
 ...
 while (@row=$sth->fetchrow_array) {
  ...
  $cart_content = $cart_content.'<TD>'.$qty.'</TD>
		...
		if ($price!=$discount) {		
	 $cart_content = $cart_content.sprintf("%0.2f", $discount * $qty).'<font size="-1" color="red"> (You saved \$'.sprintf("%0.2f", $qty * ($price - $discount)).')</font>';
  ...
  $cart_content = $cart_content.'</TD><TD align="right" valign="top" width="10%">\$'.sprintf("%0.2f", $extprice).'</TD></TR>';
	 ...
	 $cart_content = $cart_content.sprintf("%0.2f", $price * $qty);
...
ALIGN="RIGHT">$'.sprintf("%0.2f",$shipcost).'</TD></TR>';
  ...
  $cart_content = $cart_content.'<TR>
   						<TD COLSPAN="2" ALIGN="LEFT"></TD>
   	<TD COLSPAN="2" BGCOLOR="#decfde" ALIGN="RIGHT">'.$shipstate.' sales tax
</TD><TD COLSPAN="1" ALIGN="RIGHT">$'.sprintf("%0.2f",$__TAX_STATES__{uc($shipstate)}*($subtotal+$shipcost)).'</TD></TR>
  <TR><TD COLSPAN="4"><HR></TD></TR>';
  }
  ...
  $cart_content = $cart_content.'<TR>
					<TD COLSPAN="2" ALIGN="LEFT"></TD>
  <TD COLSPAN="2" ALIGN="RIGHT" BGCOLOR="#decfde">Total</TD><TD
COLSPAN="1" ALIGN="RIGHT"><B><FONT COLOR="Blue">$'.sprintf("%0.2f",$__TAX_STATES__{uc($shipstate)}*($subtotal+$shipcost)+$subtotal+$shipcost).'</B></FONT></TD></TR>';
